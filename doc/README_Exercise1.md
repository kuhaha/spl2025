...
echo '研究分野:「'. $title. '」' . PHP_EOL; // ドット演算子で繋いだ文字列を出力
echo "研究分野:「{$title}」" . PHP_EOL; // 変数を埋め込んだ文字列（ダブルクォート文字列）出力
/*出力：
 研究分野:「複合AIシステム」
 研究分野:「複合AIシステム」
*/
...
/*
 ...
 %03d: 3桁の整数としてフォーマット。不足の桁数をゼロ「0」で足す。例：1->003, 12->012
 %f: 小数(float)としてフォーマット 
 %.3f: 小数点以下3桁の小数としてフォーマット。不足の桁数をゼロ「0」で足す。例：2.5->2.500, 1->1.000
*/ 
...
printf("学籍番号：23RS%03d, 氏名：%s, GPA：%.3f". PHP_EOL, 17, '産大はなこ', 3.1);
/*出力例：
学籍番号：23RS017, 氏名：産大はなこ, GPA：3.100
*/
```
